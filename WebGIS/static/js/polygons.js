...
function initMap() {
  ...
  function loadMapShapes() {
    // load outline polygons from a GeoJson file
    map.data.loadGeoJson(
      "/static/data/states_copy1.json",
      { idPropertyName: "STATE" }
    );
    // wait for the request to complete by listening for the first 
    // feature to be added
  ...
  function styleFeature(feature) {
    const low = [5, 79, 54]; // color of smallest datum
    const high = [351, 83, 34]; // color of largest datum

    //const low = [15, 30, 5]; // color of smallest datum
    //const high = [250, 120, 100]; // color of largest datum
    ...
    for (let i = 0; i < 3; i++) {
      // calculate an integer based on the delta
    ...
    let outlineWeight = 0.2,
      zIndex = 1;
    if (feature.getProperty("state") === "hover") {
      outlineWeight = zIndex = 1.0;
    }
    return {
